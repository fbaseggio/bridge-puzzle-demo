# Play Again Replay Fix Summary + Diffs

## Summary
This patch changes replay divergence to use **EQC alt classes across buckets** (not restricted to same chosen bucket), while keeping gameplay/policy logic unchanged.

### Core behavior updates
- Replay divergence now selects an **untried alt class** at a decision index regardless of original `chosenBucket`.
- Replay forcing now stores `forcedClassId` and resolves a legal `forcedCard` at replay time by matching current legal cards to that class.
- Availability reason text changed from `no-untried-same-bucket-alternatives` to `no-untried-alternatives`.
- Tried-checking for availability now matches alt-class coverage by `problemId + decisionIndex + altClassId` (bucket-agnostic match over existing key strings).
...
## Diff: `src/demo/playAgain.ts`
```diff
diff --git a/src/demo/playAgain.ts b/src/demo/playAgain.ts
index 1551c01..facdcd1 100644
--- a/src/demo/playAgain.ts
+++ b/src/demo/playAgain.ts
@@ -4,19 +4,33 @@ export function triedAltKey(problemId: string, decisionIndex: number, chosenBuck
...
+    return { ok: false, reason: 'no-untried-alternatives' };
   }
...
## Diff: `src/demo/main.ts`
```diff
diff --git a/src/demo/main.ts b/src/demo/main.ts
index c3b8329..f9160b4 100644
--- a/src/demo/main.ts
+++ b/src/demo/main.ts
...
@@ -567,12 +621,12 @@ function appendTranscriptDecisions(before: State, events: EngineEvent[]): void {
...
@@ -680,17 +734,21 @@ function refreshThreatModel(problemId: string, clearLogs: boolean): void {
...
@@ -704,6 +762,7 @@ function resetGame(seed: number, reason: string): void {
...
@@ -725,6 +784,7 @@ function selectProblem(problemId: string): void {
...
+  runPlayCounter = 0;
   ...
   playAgainUnavailableReason = null;
   playAgainLastCandidateIndex = null;
@@ -830,7 +890,7 @@ function runTurn(play: Play): void {
     }
...
+    playAgainUnavailableReason = availability.ok ? null : (availability.reason ?? 'no-untried-alternatives');
     ...
     logs = [
       ...logs,
@@ -858,7 +918,7 @@ function startPlayAgain(): void {
...
@@ -869,39 +929,49 @@ function startPlayAgain(): void {
...
+    playAgainUnavailableReason = 'no-untried-alternatives';
     ...
     logs = [...logs, `[PLAYAGAIN] availability ok=false reason=${playAgainUnavailableReason}`].slice(-500);
     render();
@@ -922,10 +992,12 @@ function startPlayAgain(): void {
     },
     cursor: 0,
     divergenceIndex,
-    forcedCard
...
+  runPlayCounter = 0;
   ...
   playAgainUnavailableReason = null;
   playAgainLastCandidateIndex = null;
@@ -934,7 +1006,12 @@ function startPlayAgain(): void {
   deferredLogLines = [];
   unfreezeTrick(false);
...
@@ -1118,6 +1195,7 @@ function renderStatusPanel(view: State): HTMLElement {
...
## Diff: `test/engine.test.ts`
```diff
diff --git a/test/engine.test.ts b/test/engine.test.ts
index cabe7ac..fa5bb25 100644
--- a/test/engine.test.ts
+++ b/test/engine.test.ts
@@ -5,6 +5,7 @@ import { computeDefenderLabels, initThreatContext, type DefenderLabels } from '.
