# DFS Replay Termination Fix

## Files / Functions Changed
- src/demo/playAgain.ts
  - added: markDecisionCoverage(...)
  - added: computeCoverageCandidates(...)
  - kept/updated: hasUntriedAlternatives(...), divergenceCandidates(...)
- src/demo/main.ts
  - runTurn(...): abort replay on user class mismatch and re-enter DFS selection
  - runTurn(... success path): update persistent coverage and compute availability from recorded coverage
  - startPlayAgain(...): DFS candidate selection from recordedRemaining	riedByIdx, deterministic forced choice, pre-mark tried
- src/core/types.ts
  - ReplayState: added forcedClassId
- src/core/engine.ts
  - replay forcing resolves forcedClassId to legal forcedCard at divergence
- test/engine.test.ts
  - added coverage termination test
  - added/kept p004 replay branch tests
...
@@ -110,13 +118,18 @@ type GameSnapshot = {
...
@@ -259,6 +272,7 @@ function cloneStateForLog(src: State): State {
...
@@ -276,6 +290,7 @@ function cloneStateForLog(src: State): State {
           cursor: src.replay.cursor,
           divergenceIndex: src.replay.divergenceIndex,
           forcedCard: src.replay.forcedCard,
+          forcedClassId: src.replay.forcedClassId,
...
@@ -407,6 +422,15 @@ function logLinesForStep(before: State, attemptedPlay: Play, events: EngineEvent
...
@@ -424,9 +448,11 @@ function logLinesForStep(before: State, attemptedPlay: Play, events: EngineEvent
           );
         }
       }
...
+      }
       ...
       if (
@@ -440,12 +466,15 @@ function logLinesForStep(before: State, attemptedPlay: Play, events: EngineEvent
...
@@ -460,32 +489,45 @@ function logLinesForStep(before: State, attemptedPlay: Play, events: EngineEvent
...
@@ -515,24 +557,28 @@ function logLinesForStep(before: State, attemptedPlay: Play, events: EngineEvent
       );
     }
...
@@ -547,19 +593,31 @@ function appendTranscriptDecisions(before: State, events: EngineEvent[]): void {
...
@@ -567,12 +625,33 @@ function appendTranscriptDecisions(before: State, events: EngineEvent[]): void {
...
@@ -680,17 +759,21 @@ function refreshThreatModel(problemId: string, clearLogs: boolean): void {
...
@@ -698,12 +781,15 @@ function resetGame(seed: number, reason: string): void {
...
@@ -725,13 +811,16 @@ function selectProblem(problemId: string): void {
...
+  replayCoverage.representativeByIdx.clear();
   ...
   deferredLogLines = [];
   unfreezeTrick(false);
@@ -757,6 +846,8 @@ function runTurn(play: Play): void {
...
@@ -825,17 +916,19 @@ function runTurn(play: Play): void {
     };
...
+    const candidates = computeCoverageCandidates(replayCoverage, allowedIndices);
+    playAgainAvailable = candidates.length > 0;
+    playAgainUnavailableReason = playAgainAvailable ? null : 'exhausted-all-variations';
...
+      `[PLAYAGAIN] candidates=[${candidates.map((c) => c.index).join(',')}]`,
...
@@ -856,11 +949,25 @@ function runTurn(play: Play): void {
...
+  if (!playAgainAvailable) {
     ...
     render();
     return;
@@ -869,39 +976,32 @@ function startPlayAgain(): void {
...
+  if (forcedClass) {
+    const tried = replayCoverage.triedByIdx.get(divergenceIndex) ?? new Set<string>();
+    tried.add(forcedClass);
+    replayCoverage.triedByIdx.set(divergenceIndex, tried);
...
+    playAgainUnavailableReason = 'exhausted-all-variations';
     ...
     logs = [...logs, `[PLAYAGAIN] availability ok=false reason=${playAgainUnavailableReason}`].slice(-500);
     render();
@@ -922,10 +1022,12 @@ function startPlayAgain(): void {
     },
     cursor: 0,
     divergenceIndex,
-    forcedCard
...
+  runPlayCounter = 0;
   ...
   playAgainUnavailableReason = null;
   playAgainLastCandidateIndex = null;
@@ -934,7 +1036,12 @@ function startPlayAgain(): void {
   deferredLogLines = [];
   unfreezeTrick(false);
...
@@ -1118,6 +1225,7 @@ function renderStatusPanel(view: State): HTMLElement {
...
@@ -751,4 +799,139 @@ describe('bridge engine v0.1', () => {
